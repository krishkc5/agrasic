...
- Optional RV32I controller variant layered above the measurement FSM for programming policy
...
8. Optional RV32I controller architecture collateral

- If the design variant includes an RV32I controller, document it as a programming/policy layer above the existing measurement engine.
- Reference RTL entrypoints: `agriasic_rv32i_control_shell.sv` and `agriasic_digital_rv32i_top.sv`.
- Keep the measurement FSM as the real-time timing engine for excitation, settle, sample, and accumulation.
- Use a small ROM or tiny RAM for program memory if needed.
...
- MAS includes the optional RV32I controller variant, its diagram, and the explicit RV32I RTL entrypoints.
